...
{
	// sizeof(ChineseFont) / sizeof(OLED_ChineseFontType) : 用整个数组的大小除以单个类型的大小，得到字库数组的元素个数。
...
void OLED_DisplayChinese(uint8_t Lines, uint8_t Columns, const char *cnstr)
{
	...
	while(*cnstr)
	{
		if ((uint8_t)*cnstr >= 0x80)  // 判断是否为中文字符.
		{
